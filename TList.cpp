...
{
	TIterator<TListItem<T>, T> it = begin();
...
template <class T>
void TList<T>::InsertAtIterator(std::shared_ptr<T> &obj, TIterator<TListItem<T>, T> Iterator) {
...
template <class T>
std::shared_ptr<T> TList<T>::EraseAtIterator(TIterator<TListItem<T>, T> Iterator)
{
	std::shared_ptr<TListItem<T>> tmp = *Iterator;
...
std::shared_ptr<TListItem<T>> TList<T>::operator[](uint num) { // доступ к элементу списка по номеру через оператор []
	TIterator<TListItem<T>, T> it = TIterator<TListItem<T>, T>(head);
